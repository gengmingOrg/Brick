...
$.custom.form.submit =function(vs){
	...
	//0: 新增 1:更新
	var ejectState = "0";
	if (undefined !== EPM.tools.request.getParams().instance) {
		 ejectState = "1";
	}
	var ejectTenantId = $("#tenantId").attr("value");
	var processInstanceId = $.custom.getFieldValue("processInstanceId");
	var projectName = $("label[data-fieldcode='name']").siblings("div").find("input").val();
	var chargePeople = $("label[data-fieldcode='用户信息']").siblings("div").find("input").val();
	var ejectInstanceId = vs.data;
	//0: 暂存 1:提交
	var submitType = "1";
	//查询项目编号
    var findUrl = "/epm/rest/entity/instance/project/"+ejectInstanceId+"?tenantId="+ejectTenantId+"&fieldCodes=projectNo";
	$.ajax({
		url : findUrl,
		type: "GET",
		dataType : "json",
		...
		success: function(data){
			var projectNo = data.data.projectNo;
			document.getElementsByTagName("iframe")[0].contentWindow.submitData(projectName,chargePeople,ejectInstanceId,ejectTenantId,projectNo,ejectState,submitType);
		}
	});
...
window.toPage=function(){
	...
	var repuestUrl="/process/startProcessByModelKey?modelKey=oms_epm_execute&loginId="+$('#loguser').val()+"&tenant_id="+$('#tenantId').val()+"&autoClaim=true&instanceId="+vs.data+"&manageOrganizationId="+manageOrganizationId+"&type="+type+"&passVp="+passVp+"&isKY="+isKY+"";
	...
	if(taskId==undefined){
		$.ajax({
			  ...
			  success:function(pIdData){
				...
				debugger
				if(item.success){
					var copUrl="/worktask/completeTask?loginId="+$('#loguser').val()+"&taskId="+item.taskId+"&action=pass&message=&tenant_id="+$('#tenantId').val()+"";
						...
						$.ajax({
							  ...
							  success:function(data){
								...
								if(itm.success){
									...
									$.shade(false);
									$.epm.snackbar("流程启动失败");
									var param_str = (location.href.indexOf('_i') != -1 ? '':('_i' + vs.data)) + '?taskId=' + item.taskId + '&flowInstId=' + item.processInstanceId;
		...
		var copUrl="/worktask/completeTask?loginId="+$('#loguser').val()+"&taskId="+taskId+"&action=pass&message=&tenant_id="+$('#tenantId').val()+"";
...
$("[data-entity-fieldcode=processInstanceStatus]").not($("label")).val("2");
...
$.ajax({
	...
	success : function(data) {
		debugger
		if($.custom.getFieldValue("isScientific") == 'yes'
		&& ($.custom.getFieldValue("projectDescription") !=""
		|| $.custom.getFieldValue("specialRequire") !=""
		|| $.custom.getFieldValue("purchaseDescription") !="")
		|| $.custom.getFieldValue("peopleCostDescription") !=""){
			$.shade(false);
			$.epm.snackbar("科研项目中不能填写项目描述、项目投入特殊要求/风险、采购说明、人员投入说明！");
			return;
		}

		if($.custom.getFieldValue("manageOrganizationId")=="root" || $.custom.getFieldValue("manageOrganizationId") == "bonc01"){
			$.shade(false);
			...
			function(){
				$.shade(true);
				validate ? ( submitFlag = false,$.custom.form.subflag = 1,$('form').submit() ) : ($.epm.snackbar('数据填写有误，请检查后再提交！'),$.shade(false));
			},function(){
				return;
			})
		}else{
